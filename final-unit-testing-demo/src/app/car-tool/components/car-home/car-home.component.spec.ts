...
describe('CarHomeComponent', () => {
  ...
  let fakeCarsServiceAll: any;
  let fakeCarsServiceAppend: any;
  let fakeCarsServiceReplace: any;
  let fakeCarsServiceDelete: any;
  ...
  const fakeCarsService = {
    ...
    append(car: NewCar) {
      return of(this._cars = this._cars.concat({
        ...car, id: Math.max(...cars.map(c => c.id) as number[], 0) + 1,
      }));
    },
    replace(car: Car) {
  ...
  beforeEach(() => {
    ...
    fakeCarsServiceAll = jest.spyOn(fakeCarsService, 'all');
    fakeCarsServiceAppend = jest.spyOn(fakeCarsService, 'append');
    fakeCarsServiceReplace = jest.spyOn(fakeCarsService, 'replace');
    fakeCarsServiceDelete = jest.spyOn(fakeCarsService, 'delete');
  ...
  it('should refresh cars', () => {
    ...
    expect(fakeCarsServiceAll).toHaveBeenCalled();
  ...
  it('should append car', () => {
    ...
    expect(fakeCarsServiceAppend).toHaveBeenCalledWith(car);
    expect(fakeCarsServiceAll).toHaveBeenCalled();
  ...
  it('should replace car', () => {
    ...
    expect(fakeCarsServiceReplace).toHaveBeenCalledWith(car);
    expect(fakeCarsServiceAll).toHaveBeenCalled();
    expect(component.cars.length).toBe(2);
  ...
  it('should delete car', () => {
    ...
    expect(fakeCarsServiceDelete).toHaveBeenCalledWith(carId);
    expect(fakeCarsServiceAll).toHaveBeenCalled();
